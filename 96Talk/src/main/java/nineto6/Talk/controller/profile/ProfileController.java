package nineto6.Talk.controller.profile;
...
import nineto6.Talk.controller.profile.swagger.ProfileControllerDocs;
...
public class ProfileController implements ProfileControllerDocs {
    ...
    public ResponseEntity<ApiResponse> getSearchProfile(
        ...
        profileSearchRequest.setKeyword(keyword);

        if(!ObjectUtils.isEmpty(page) && !ObjectUtils.isEmpty(recordSize) && !ObjectUtils.isEmpty(pageSize)) {
            profileSearchRequest.setPage(page);
            profileSearchRequest.setRecordSize(recordSize);
            profileSearchRequest.setPageSize(pageSize);
        }

        PagingResponseDto<ProfileResponse> searchProfile = profileService.getSearchProfile(profileSearchRequest);
